const SYMBOLS = {
  ...
  function Board (options){
    ...
    function render(){
      ...
      function htmlQ1(){
        return `<div id="view1"><h2>Jogo da velha\n</h2>
        ${buttonHTML(1, "1player", "Contra maquina")}
        ${buttonHTML(1, "2players", "Dois jogadores")}
        </div>`
      }
  
      function htmlQ2(){
        const html2=`<div id="view2"><h3>${!state.players[1].isComputer? "Jogador1, <br />" : ""}Escolha entre X ou O</h3>
        ${buttonHTML(2, "X", "X")}
        ${buttonHTML(2, "O", "O")}`
        return html2
      }
      ...
      function htmlGame (){
        ...
        const playerName = state.game.turn === 0 ? 'Jogador1' : state.players[1].isComputer ? 'Computador' : 'Jogador2'
  
        let htmlBefore = `<h3>Movimento: ${moveNumber} ${htmlSpaces(5)} Vez: ${playerName}</h3>`
        let board = state.game._gameBoard.reduce(function(acc,curr,rowIndex){
          ...
          let htmlAfter = `<h4>Placar: ${htmlSpaces(1)} Jogador1 - ${state.players[0].score} ${htmlSpaces(2)} ${state.players[1].isComputer? "Computador" : "Jogador2" } - ${state.players[1].score}</h4>`
      ...
      function htmlGameEnd (){
        ...
        if(result !== RESULT.tie)
          resultText = getPlayerName(result) + " Ganhou!"
  
        let htmlBefore = `<h3>${resultText} ${htmlSpaces(2)} Clique para reiniciar </h3> `
        let board = state.game._gameBoard.reduce(function(acc,curr,rowIndex){
          ...
          let htmlAfter = `<h4>Placar: ${htmlSpaces(1)} Jogador 1 - ${state.players[0].score} ${htmlSpaces(2)} ${state.players[1].isComputer? "Computador" : "Jogador 2" } - ${state.players[1].score}</h4>`
